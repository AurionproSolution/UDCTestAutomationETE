...
/** Lease Details residual % after first Calculate (matches sanity pattern; keep ≤ cash vs GST rules on this build). */
const FL_RESIDUAL_PERCENT = "10";
...
test(
  ...
  async ({ page }) => {
    ...
    // Asset #2: MotoCheck — re-enable when stable (Search & Add → BAGGED → Submit; summary stays open until cross).
    // await assetDetailsPage.performMotoCheckAssetSearchAndAdd({
    //   enterNumber: "BAGGED",
    // });

    await addAssetPage.clickCrossButton();
    // Lease Details: re-apply Asset Type + Condition after closing summary (avoids BR_ASSET_MSG_16 if fields cleared).
    await assetDetailsPage.enterAsset("Car and Light Commercial /");
    await assetDetailsPage.selectCondition("Used");
    ...
    await assetDetailsPage.enterResidualValuePercentFinanceLease(FL_RESIDUAL_PERCENT);
    await assetDetailsPage.interestRate("4");
    await assetDetailsPage.enterOriginationReferenceFinanceLease("Test Orig Ref 123", true);
    await assetDetailsPage.clickCalculateButton();
 
    // Add Ons & Accessories — single session with insurance regression (Yes radios, months/amount, validation, recovery).
    // Uses completeFlStandardQuoteAddOnsSingleSessionWithInsuranceRegression so FL runs insurance while
    // SKIP_INSURANCE_ADDONS_AUTOMATION stays true for other callers on DOAddOnsAccessoriesPage.
    const addOnsAccessoriesPage = new DOAddOnsAccessoriesPage(page);
    await addOnsAccessoriesPage.completeFlStandardQuoteAddOnsSingleSessionWithInsuranceRegression();
