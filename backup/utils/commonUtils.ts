...
export class CommonUtils {
   ...
   * @param actionTimeoutMs optional visibility wait before fill (e.g. long IdP / captcha waits)
   */
  async fill(locator: Locator, text: string, actionTimeoutMs?: number): Promise<void> {
    await locator
      .waitFor({ state: "visible", timeout: actionTimeoutMs ?? 10_000 })
      .catch(() => {});
    await this.highlight(locator);
    await locator.clear();
